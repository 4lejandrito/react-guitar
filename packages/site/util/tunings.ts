import { get, midi } from '@tonaljs/note'
...
const tunings: Tuning[] = [
  ...
  ...tuning,
  notes: tuning.notes.map(name => midi(name) ?? 0).reverse()
}))

export default tunings

export const toString = (tuning: number[]) =>
  tuning
    .map(midi =>
      midiToNoteName(midi, { pitchClass: true, sharps: true }).padEnd(2, ' ')
    )
    .reverse()
    .join(' ')
